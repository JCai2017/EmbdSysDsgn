// i_typed_tranceiver_os.sh: template (macro) for a typed tranceiver_os interface
//
...
#ifndef I_TYPED_TRANCEIVER_OS_SH
#define I_TYPED_TRANCEIVER_OS_SH


#define DEFINE_I_TYPED_TRANCEIVER_OS(typename, type)			\
									\
interface i_ ## typename ## _tranceiver_os					\
{									\
    note _SCE_STANDARD_LIB = { "i_tranceiver_os", #typename, #type };	\
									\
    void send(type d);							\
    void receive(type *d);						\
};
...
#endif /* I_TYPED_TRANCEIVER_OS_SH */


// EOF i_typed_tranceiver_os.sh
