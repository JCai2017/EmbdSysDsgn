// i_typed_sender_os.sh: template (macro) for a typed sender_os interface
//
...
// interface rules:
//
// - a connected thread acts as a sender_os
...
#ifndef I_TYPED_SENDER_OS_SH
#define I_TYPED_SENDER_OS_SH


#define DEFINE_I_TYPED_SENDER_OS(typename, type)				\
									\
interface i_ ## typename ## _sender_os					\
{									\
    note _SCE_STANDARD_LIB = { "i_sender_os", #typename, #type };		\
									\
    void send(type d);							\
};


#endif /* I_TYPED_SENDER_OS_SH */


// EOF i_typed_sender_os.sh
